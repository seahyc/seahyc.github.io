...
Fresh mock launch records whether the task has been opened or attempted in this browser. Familiar retries remain practice. The page cannot detect prior exposure elsewhere, enforce tool rules, or authenticate peer review. Coding-assessment qualification uses the latest five fresh mocks in a 21-day window: at least four must be strong passes, the latest three must pass consecutively, strong passes must span three task families and three days, and one must retain a ten-minute review buffer. A strong pass also requires scores of at least 2/3 for correctness, algorithm and complexity, testing, and explanation, plus a structured postmortem. Rehearsal thresholds require every anchored dimension at least 2/3, written evidence, specific feedback, and learner-reported peer review within 30 days. A subsequent failed rehearsal invalidates older passing review evidence until repaired. There is no automatic spoken grading or recording, and visible browser tests are not held-out evaluation.
