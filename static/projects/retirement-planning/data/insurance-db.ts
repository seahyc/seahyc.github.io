...
import type { CoverageBenefit, InsurancePlanLike, InsuranceRiderOption, TreatmentClass } from "../policy/medical-schemes.js";
...
function inferWardTier(label: string): "public" | "b1" | "a" | "private" {
  if (/standard/i.test(label)) return "public";
  if (/\bB\b|Lite|Basic/i.test(label)) return "b1";
  if (/\bA\b|Advantage|Plus|Plan 2/i.test(label) && !/Private|Premier|Preferred|Plan 1/.test(label)) return "a";
  return "private";
}

export const INSURANCE_RIDER_CATALOG: Record<string, RiderSeed[]> = {
  public: [
    {
      id: "none",
      label: "No rider",
...
function buildPlanCatalogEntries(insurers: Record<string, ProviderPlans>): InsuranceCatalogEntry[] {
  return Object.entries(insurers).flatMap(([provider, providerRecord]) =>
    Object.entries(providerRecord.plans).map(([planName, plan]) => {
      ...
      return {
        ...
        sourceRefs: [providerRecord.sourceId, plan.sourceId].filter(Boolean) as string[],
        compatibility: {
          ...
          outpatientCancerCoverage: (plan.claimPathRules?.outpatientCancerDrugCoverage as InsuranceCatalogEntry["compatibility"]["outpatientCancerCoverage"]) || "cdl-and-non-cdl",
          claimPathTags: [
...
function buildRiderCatalogEntries(): InsuranceCatalogEntry[] {
  return Object.entries(INSURANCE_RIDER_CATALOG).flatMap(([provider, riders]) =>
    riders.map((rider) => ({
      skuKind: "rider" as const,
      skuId: rider.sku,
      provider,
...
UNIFIED_INSURANCE_DB.catalog = [
  ...buildPlanCatalogEntries(UNIFIED_INSURANCE_DB.insurers as Record<string, ProviderPlans>),
