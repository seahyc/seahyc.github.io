...
export function createProfile(state: AppState): ProfileRecord {
...
export function duplicateProfile(state: AppState, profileId: string | null): ProfileRecord | null {
  ...
  const duplicatedPlans: PlanData[] = plans.map((plan, index) => ({
    ...deepClone(plan),
    id: newId("plan"),
    profileId: targetProfileId,
    name: index === 0 ? `${plan.name} copy` : plan.name,
  }));
  const duplicatedProfile: ProfileRecord = {
...
export function deleteProfile(state: AppState, profileId: string | null): boolean {
  if (state.profiles.length === 1 || !profileId) return false;
