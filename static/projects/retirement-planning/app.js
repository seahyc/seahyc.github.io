...
function render() {
  ...
  `;
    ...
    const expertFold = app.querySelector("#rp-expert-fold");
    if (expertFold) {
...
function renderFuturesTopline(bundle, profile) {
    ...
    <div class="rp-futures" id="rp-futures">
      <div class="rp-futures-main">
        <div>
          <div class="rp-futures-hero">${t(`<b>${fut.okOf100}</b> / 100 个未来里，钱够用一辈子`, `<b>${fut.okOf100}</b> / 100 futures, your money outlives you`)}</div>
        </div>
      ...
      <div class="rp-futures-side">
        ...
        <div class="rp-futures-fancaption">${t("100 个未来里 80 个落在绿色范围内 — 也可能落在外面", "80 of 100 futures fall inside the band; some fall outside")}</div>
      </div>
...
const DISEASE_LABELS = new Map(SUPPORTED_DISEASES.map((d) => [d.key, d.label]));
...
const DESTINATION_LABELS_ZH = {
    japan: "日本", europe: "欧洲", africa: "非洲", us: "美国", sea: "东南亚",
};
function diseaseLabel(key) {
    if (getLang() === "zh")
        return DISEASE_LABELS_ZH[key] ?? DISEASE_LABELS.get(key) ?? key;
    return DISEASE_LABELS.get(key) ?? key;
}
function getParkedDate(cardId) {
    try {
        const map = JSON.parse(window.localStorage.getItem(PARKED_KEY) || "{}");
...
function setParkedDate(cardId, iso) {
    try {
        const map = JSON.parse(window.localStorage.getItem(PARKED_KEY) || "{}");
        map[cardId] = iso;
...
function renderAgeDelta(bundle, profile) {
    const plan = bundle.plan;
    ...
    const gapWarn = altMinLiquid < 0
        ...
        : "";
    return `<div class="rp-futures-delta">${t(`改到 ${candidateAge} 岁开始领：<b>${altFut.okOf100}/100</b> 个未来够用（${okDeltaStr}），每月${payoutDelta >= 0 ? "多" : "少"} <b>${payoutStr}</b>${gapWarn}`, `Start at ${candidateAge} instead: <b>${altFut.okOf100}/100</b> futures last (${okDeltaStr}), <b>${payoutStr}</b> ${payoutDelta >= 0 ? "more" : "less"}/mo${gapWarn}`)}</div>`;
}
function renderLockDelta(bundle, profile, maxLock) {
    const plan = bundle.plan;
    ...
    return `<div class="rp-futures-delta">${t(`${dir} <b>${amt}</b> 进 RA：<b>${altFut.okOf100}/100</b> 个未来够用（${okDeltaStr}），每月${payoutDelta >= 0 ? "多" : "少"} <b>${currency.format(Math.abs(payoutDelta))}</b>`, `${dir} <b>${amt}</b> into RA: <b>${altFut.okOf100}/100</b> futures last (${okDeltaStr}), <b>${currency.format(Math.abs(payoutDelta))}</b> ${payoutDelta >= 0 ? "more" : "less"}/mo`)}</div>`;
}
function renderZoomRitual(cardId, bundle, profile, isPlan) {
    ...
    const survivalAt = (target) => base.points.find((p) => p.age === target)?.survival ?? null;
    const s85 = survivalAt(85);
    const s90 = survivalAt(90);
    const outside = (s85 !== null && s90 !== null)
        ? t(`100 个像您一样的${who}（${currentAge} 岁）：约 <b>${Math.round(s85 * 100)}</b> 个活过 85，<b>${Math.round(s90 * 100)}</b> 个活过 90。`, `Of 100 ${who} like you (age ${currentAge}): about <b>${Math.round(s85 * 100)}</b> live past 85, <b>${Math.round(s90 * 100)}</b> past 90.`)
    ...
    if (profile.smoking === "current")
        nudges.push({ label: t("现在吸烟", "current smoker"), dir: "up" });
    else if (profile.smoking === "former")
        nudges.push({ label: t("曾经吸烟", "former smoker"), dir: "up" });
    if (profile.alcohol === "heavy")
        nudges.push({ label: t("大量饮酒", "heavy drinking"), dir: "up" });
    if (profile.selfRatedHealth === "poor")
        nudges.push({ label: t("自评健康差", "self-rated poor health"), dir: "up" });
    else if (profile.selfRatedHealth === "good")
        nudges.push({ label: t("自评健康良好", "self-rated good health"), dir: "down" });
    if (profile.frailty === "frail")
        nudges.push({ label: t("身体虚弱", "frail"), dir: "up" });
    else if (profile.frailty === "prefrail")
        nudges.push({ label: t("偏虚弱", "pre-frail"), dir: "up" });
    if (profile.mobility !== "independent")
        nudges.push({ label: t("行动需协助", "mobility needs help"), dir: "up" });
    if (profile.cognition !== "normal")
        nudges.push({ label: t("认知有状况", "cognition affected"), dir: "up" });
    ...
    if (iv.exerciseUpgrade)
        nudges.push({ label: t("加强运动计划", "exercise upgrade"), dir: "down" });
    if (iv.bpControl)
        nudges.push({ label: t("控制血压", "BP control"), dir: "down" });
    if (iv.smokingCessation && profile.smoking === "current")
        nudges.push({ label: t("戒烟计划", "quit-smoking plan"), dir: "down" });
    ...
    const premortemHtml = premortemRows.length
        ? premortemRows.map((r) => `<div class="rp-premortem">${t(`约 <b>${r.pct}</b> / 100 个未来因<b>${r.label}</b>在 ${r.medBreach} 岁左右变紧 → ${r.fix}`, `~<b>${r.pct}</b> of 100 futures tighten from <b>${r.label}</b> around age ${r.medBreach} → ${r.fix}`)}</div>`).join("")
...
function bindDecisionRail(bundle, profile) {
    void bundle;
    void profile;
    if (!state)
        return;
    app.querySelectorAll("[data-rail-plan]").forEach((btn) => btn.addEventListener("click", async () => {
        if (!state)
            return;
        const kind = btn.dataset.railPlan;
        if (kind !== "standard" && kind !== "escalating" && kind !== "basic")
            return;
        getActivePlan(state).cpfPlan = kind;
        syncActivePlanConstraints(state);
        await persist();
    }));
    app.querySelectorAll("[data-rail-commit]").forEach((btn) => btn.addEventListener("click", () => {
        if (btn.dataset.railPlanFinal) {
            const final = app.querySelector(`[data-rail-final="${btn.dataset.railCommit}"]`);
            if (final)
                final.hidden = false;
            return;
        }
    ...
    app.querySelectorAll("[data-rail-confirm]").forEach((btn) => btn.addEventListener("click", () => {
        showToast("success", t("已记录 · 选定 30 天后不能更改", "Noted — final 30 days after enrolment"));
        const final = app.querySelector(`[data-rail-final="${btn.dataset.railConfirm}"]`);
        if (final)
            final.hidden = true;
    }));
    app.querySelectorAll("[data-rail-park]").forEach((btn) => btn.addEventListener("click", () => {
        const id = btn.dataset.railPark;
        if (!id)
            return;
        const iso = new Date().toISOString().slice(0, 10);
        setParkedDate(id, iso);
        const chip = app.querySelector(`[data-rail-parkchip="${id}"]`);
        if (chip) {
            chip.hidden = false;
...
function renderInsuranceReviewAlert(profileRecord, bundle) {
    ...
    <div class="rp-alert rp-alert-warning rp-insurance-review-card">
      ...
      <div>${t(`${age} 岁、有「${escapeHtml(conditions)}」、且只选了公积金公共基础住院保障，未来一次住院仍可能留下不小的现金账单。目前模型估计在重大冲击前每年自付约 ${currency.format(first.medicalCash)}。`, `At age ${age}, with ${escapeHtml(conditions)} and only public-baseline hospital coverage selected, a future hospitalisation could still leave meaningful cash bills. The current model estimates about ${currency.format(first.medicalCash)}/year out of pocket before any major shock.`)}</div>
      <div><strong>${t("下一步该做什么：", "What to do Monday morning:")}</strong> ${t("向保险顾问或保险公司询问：现在还能买哪些综合健保计划（Integrated Shield）、有哪些除外条款、以及在下个生日前的年保费是多少。", "Ask an insurance adviser or provider what Integrated Shield coverage is still available, what exclusions apply, and what the annual premium would be before your next birthday.")}</div>
    </div>
  `;
}
function renderIncomeGapAlert(bundle) {
    const first = bundle.result.rows[0];
    if (!first)
        return "";
    const monthlyIncome = Math.round(first.grossIncomeAnnual / 12);
    ...
    <div class="rp-alert rp-alert-warning rp-income-gap-card">
      ...
      <div>${t(`每月收入（${currency.format(monthlyIncome)}/${m}）不足以支付基本开销（${currency.format(monthlyBasicSpend)}/${m}）。缺口：${currency.format(Math.abs(monthlyGap))}/${m}。`, `Your monthly income (${currency.format(monthlyIncome)}/${m}) does not cover basic needs (${currency.format(monthlyBasicSpend)}/${m}). Shortfall: ${currency.format(Math.abs(monthlyGap))}/${m}.`)}</div>
    </div>
  `;
}
