import { buildBaselineSurvival } from "./mortality-baseline.js";
...
function buildAdjustedSurvival(points, multiplier) {
    const out = [];
    let survival = 1;
    points.forEach((point, index) => {
        if (index > 0)
            survival *= Math.max(0, 1 - Math.min(0.99, point.qx * multiplier));
        out.push(survival);
    });
    return out;
}
...
function survivalQuantileAge(ages, survival, threshold) {
    for (let i = 0; i < survival.length; i += 1) {
        if ((survival[i] ?? 1) <= threshold)
            return ages[i] ?? ages[ages.length - 1] ?? 0;
    }
...
function modalDeathAge(ages, survival) {
    let bestDrop = -1;
