...
function drawBarSeries(ctx, width, height, padding, labels, seriesIndex, seriesCount, data, color, yMax, yMin = 0) {
    const plotWidth = width - padding.left - padding.right;
    const plotHeight = height - padding.top - padding.bottom;
...
export function renderChart(canvas, config) {
    ...
    const setTooltipPositionForIndex = (hoverIndex) => {
        const rect = canvas.getBoundingClientRect();
        const firstSeries = config.series[0];
        if (!firstSeries)
            return;
    ...
    const updateHover = (event, pin = false) => {
        ...
        if (pin)
            setTooltipPositionForIndex(hoverIndex);
        else
