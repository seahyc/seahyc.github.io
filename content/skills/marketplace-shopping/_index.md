...
Avoid Dynamic Island / status-bar taps (y ≲ 80–90). Use **human-like variability** where MobileCLI works. On **淘宝**, if captcha fires on automation, **stop driving** — user browses, agent parses share links/screenshots (see mobile guide **Taobao antibot**). Never enter passcodes or payment secrets.
