...
export function drawHandFeedback(ctx:CanvasRenderingContext2D,hands:{landmarks:Point[]}[],{single,decision,fresh,turn=0,reverse=false}:{single:boolean;decision:string;fresh:boolean;turn?:number;reverse?:boolean}){
 ...
 ctx.fillStyle=fresh?'#e1f0d6':'#99ad9e';ctx.textAlign='center';ctx.font='bold 18px system-ui';ctx.fillText(!fresh?(single?'SHOW YOUR HAND':'SHOW YOUR HANDS'):decision==='spraying'?'HOSE · AIM & SPRAY':decision==='point-dwell'?'HOSE · READY…':single?(labels[decision]??'WALK YOUR FINGERS'):'TWO-HAND WALKING',160,23);
