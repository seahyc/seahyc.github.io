...
export class OneHandController{
 ...
 update(hands,now,{overUi=false}={}){
  const h=hands?.length===1&&valid(hands[0])?hands[0]:null;if(!h||!Number.isFinite(now)){this.stop('lost');this.turn=0;this.filteredTurn=0;this.lastSeen=-Infinity;this.mode='lost';return this.read(now);}
