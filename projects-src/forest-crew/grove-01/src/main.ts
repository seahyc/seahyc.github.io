...
function reset(){movement.reset();fire.reset();playerHeat.reset();rescueUntil=0;$('completion').hidden=true;input?.record('scenario-reset',{scenario:'hose-baseline',seed:73011});}
...
function renderFrame(){
 ...
 latestState={...state,fire:fireState,health:heat,input:frame,impact:impact?{x:impact.x,y:impact.y,z:impact.z}:null,frameMs:dt*1000};
 if(now-lastTrace>100){lastTrace=now;input.record('world-state',{position:state.position,yaw:state.yaw,speed:state.speed,displacement:state.displacement,blocked:state.blocked,gait:state.gait,camera:state.camera,fire:fireState,health:heat,impact:latestState.impact,pressure:1,mode:frame.mode,dt,fps:env.engine.getFps(),quality:env.renderQuality(),seed:73011});}
