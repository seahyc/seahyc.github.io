...
export function createInputRuntime({canvas,container,getHeading,onEvent}:Options){
 ...
 root.innerHTML=`<div class="forest-input__status" role="status" aria-live="polite">Allow camera access, then show your hand.</div>
  ...
  <aside class="forest-input__hud" aria-label="Optional playtest and camera HUD" aria-hidden="true"><button class="forest-input__hud-close" data-action="hud-close" data-gesture>Hide settings</button><button class="forest-input__mode-action" data-action="input-mode" data-gesture>Use one-hand controls</button><div class="forest-input__camera" aria-label="Mirrored hand tracking preview"><video playsinline muted aria-hidden="true"></video><canvas width="320" height="240" aria-hidden="true"></canvas><div class="forest-input__framing">Show both hands at chest height · 0/2 detected</div></div><div class="forest-input__record" aria-live="polite"><div data-kind="local">LOCAL · WAITING</div><div data-kind="remote">REVIEW COPY · OFF</div><div>Game + camera · no audio</div><button class="forest-input__record-action" data-action="record" data-gesture>Play without recording</button></div></aside>
 ...
 function updateModeCopy(){root.dataset.inputMode=inputMode;modeButton.textContent=inputMode==='one-hand'?'Use two-hand controls':'Use one-hand controls';controlsCopy.textContent=inputMode==='one-hand'?'Walk index and middle fingers to move. Tilt your hand to turn. Point and hold to spray; open your palm to stop.':'Walk your fingers to move. Tilt both hands to turn. Point with one hand and hold the other palm open to spray.';}
 ...
 function toggleInputMode(){inputMode=inputMode==='one-hand'?'two-hand':'one-hand';updateModeCopy();setHud(false);resetCalibration();emit('input-mode',{inputMode,source:'settings'});}
 ...
 function frame(now:number):InputFrame{
  ...
  const stale=now-lastResult>STALE_MS||hands.length===0||!visible||suspended;if(stale){walker.reset();oneHand.read(now);hose.read(now);pointerTracked=false;cursor.classList.add('is-lost');clearUiAuthority();}
