import {createGestureReplay} from './gesture-replay.mjs';
...
export function createInputRuntime({canvas,container,getHeading,onEvent}:Options){
 ...
 const replayMode=import.meta.env.DEV&&new URLSearchParams(location.search).get('qa')==='1'&&new URLSearchParams(location.search).get('replay')==='1';
 let qaInjected=replayMode;
 let replayTrace:any[]=[];
 const replay=createGestureReplay({ingest:ingestLandmarks,reset:()=>{qaInjected=true;resetCalibration();hands=[];rawHands=[];lastResult=-Infinity;suspended=false;replayTrace=[];},onStop:()=>{hands=[];rawHands=[];lastResult=-Infinity;resetControllers();clearUiAuthority();drawHands([]);}});
 ...
 const emit=(type:string,data:any={})=>{const payload={version:'0.2.6',buildId:BUILD_ID,scenario:SCENARIO,fixture:qaMode,inputSource:replayMode?'landmark-replay':qaInjected?'sampled-hand-fixture':'camera',...data};if(replayMode){if(replayTrace.length<25000)replayTrace.push({t:performance.now(),type,data:payload});onEvent?.(type,payload);return;}playtest.record(type,payload);const now=performance.now();if(playtest.state==='recording'&&(criticalEvents.has(type)||now-lastFlush>=1200)){lastFlush=now;void playtest.flushTelemetry();}onEvent?.(type,payload);};
 ...
 async function startRecording(explicitConsent:boolean){
  ...
  try{if(replayMode)return;if(playtest.state!=='undecided'&&!playtest.reset())return;await playtest.optIn({explicitConsent,buildId:BUILD_ID,scenario:SCENARIO});localStorage.setItem('forest-crew-recording','on');recorder.setEnabled(true);recorder.start();countdown.textContent='● REC';emit('recording-start',{mode:explicitConsent?'explicit':'remembered-default'});}catch(error){recordingError=String(error);localLabel.textContent='LOCAL · SAVE UNAVAILABLE';countdown.textContent='REC unavailable';feedback.textContent='Recording is unavailable; play continues.';}
 }
 ...
 function ingest(sampled:any[],landmarks:RawHand[],now:number,sampleLatency=0,aspect=4/3){
  ...
  drawHands(rawHands);const hoseTelemetry=hose.read(now,{overUi});emit('hands',{timestamp:Math.round(now),latency,mode:baseMode,inputMode,control:inputMode==='one-hand'?oneHand.diagnostics(now):null,stance:hoseTelemetry.blockWalking?'hose':'walk',intentReason:hoseTelemetry.reason,walkingBlocked,inputPreference,walkingSelection:selected,aim:hoseTelemetry.aim,aspect,landmarks:rawHands});
 }
 function ingestLandmarks(landmarks:RawHand[],now:number,aspect=4/3,sampleLatency=0){
  const sampled=landmarks.flatMap(hand=>{const sample=withGestureIntent(withNavigationPoint(sampleHand(hand.id,hand.landmarks,aspect),hand.landmarks,aspect),hand.landmarks);return sample?[sample]:[];});
  ingest(sampled,landmarks,now,sampleLatency,aspect);
 }
 ...
 async function start(){
  if(started)return;started=true;if(replayMode){playtest.optOut();countdown.textContent='REPLAY · camera & recording off';setStatus('Gesture replay · camera and recording off');return;}const token=++generation;setStatus('Allow camera access. No microphone is used.');
 ...
 function frame(now:number):InputFrame{
  ...
  if(replayMode){if(document.hidden)replay.stop('interrupted');else replay.tick(now);}
 ...
 if(replayMode)(window as any).__forestGestureReplay={load:replay.load,play:()=>replay.play(performance.now()),stop:()=>replay.stop(),status:replay.status,trace:()=>replayTrace};
