...
export function createCrewClient({invite=null}:CrewClientOptions={}){
 ...
 function update(now:number,frame:PlayerFrame){
  ...
  void post('frame',{spraying:frame.spraying,progress:frame.progress,complete:frame.complete}).then(s=>{
   ...
   const jobs:Record<string,string>={fetch_hose:'fetching hose',connect_hose:'connecting hose',repair_pump:'repairing pump',operate_pump:'operating pump'};
   const activities=(s.actors??[]).map((a:any)=>`${a.id==='firefighter'?'Teal':'Amber'}: ${a.activity==='walking'?'going to '+(a.taskId?.includes('pump')?'pump':'hose'):a.taskId&&jobs[a.taskId]?jobs[a.taskId]:a.thinking?'planning':'standing by'}`).join(' · ');
   say(s.status==='stopped'?'CREW · session ended':s.error?'CREW · teammate unavailable':activities?`${s.pressure?'WATER READY':'CREW'} · ${activities}`:message?`CREW · ${String(message.text).slice(0,140)}`:'CREW · setting up your water supply…');
